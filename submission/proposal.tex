...
ROC plots the true positive rate over the false positive rate, while AUROC is interpreted as the chance that the model can distinguish between disease states.
...
These steps will be repeated for 100 iterations and the test AUROCs will be recorded.
...
The Wilcoxon test is appropriate because for each model, there is a mean AUROC for each data split, thus the mean AUROCs are matched by data splits.
