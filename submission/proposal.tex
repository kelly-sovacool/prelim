...
OTU-based machine learning (ML) models have been developed that improve sensitivity when used in complement with FIT, demonstrating the feasibility of using microbiome-based markers for diagnosis \cite{baxter_microbiota-based_2016}.
...
Taxonomic composition is often compared between communities by calculating beta diversity metrics such as the Bray-Curtis distance and plotting an ordination to visualize separation between groups, such as cancerous and healthy.
One would think that if ML models can use taxonomic composition to classify samples as cancerous or healthy,
that differences in taxonomic composition would also be reflected in an ordination.
However, patient stool samples cannot be classified as having SRNs or not using beta diversity,
as shown in the lack of distinct clustering in Fig. \ref{fig:nmds_otu}.
also in the case of obesity

hannigan: rf implies biological significance (see discussion section)
